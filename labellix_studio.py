...
class FrameExtractDialog(QDialog):
    def __init__(self, video_path, parent=None):
        ...
        layout = QVBoxLayout()
        self.info_label = QLabel(f"Video: {os.path.basename(video_path)}\nDuration: {self.duration:.2f} sec\nVideo FPS: {self.video_fps:.2f}")
        layout.addWidget(self.info_label)

        fps_layout = QHBoxLayout()
        fps_layout.addWidget(QLabel("Frames per second to extract:"))
        self.fps_spin = QSpinBox()
        self.fps_spin.setRange(1, int(self.video_fps) if self.video_fps > 0 else 30)
        self.fps_spin.setValue(10)
        self.fps_spin.valueChanged.connect(self.update_estimate)
    ...
    def update_estimate(self):
        ...
        self.estimate_label.setText(f"Estimated frames to extract: {self.estimated_frames}")
#!/usr/bin/env python
# -*- coding: utf-8 -*-
import argparse
import codecs
...
from libs.export_workflows import ExportWorkflowsMixin
from libs.classification_workflows import ClassificationWorkflowsMixin
from libs.training_workflows import TrainingWorkflowsMixin
from libs.ui_state_workflows import UIStateWorkflowsMixin
from libs.annotation_workflows import AnnotationWorkflowsMixin
from libs.mode_workflows import ModeWorkflowsMixin
...
class MainWindow(QMainWindow, WindowMixin, ExportWorkflowsMixin, TrainingWorkflowsMixin, ClassificationWorkflowsMixin, UIStateWorkflowsMixin, AnnotationWorkflowsMixin, ModeWorkflowsMixin):
    # Remove the first __init__ (if present)
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.video_progress_dialogs = {}  # video_path -> QProgressDialog
        ...
        self.license_plate_predefined_classes_file = os.path.join(
            os.path.dirname(__file__), 'data', 'predefined_classes_license_plate.txt'
        )
        ...
        # Ensure the license plate class file exists early.
        self._read_predefined_classes_file(self.license_plate_predefined_classes_file)
        ...
        if label and label not in self.label_hist:
            ...
            if self.is_license_plate_mode():
                self._write_predefined_classes_file(
                    self.license_plate_predefined_classes_file,
                    self.label_hist,
                )
    ...
    def load_license_plate_label_history(self):
        self.predefined_classes_file = self.license_plate_predefined_classes_file
        latest_predefined = self._read_predefined_classes_file(self.license_plate_predefined_classes_file)
    ...
    def _new_training_worker(self, command):
        return TrainingWorker(command, parent=self)
    ...
    def on_video_extraction_finished(self, video_path, frame_count):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                self.statusBar().showMessage(f'Failed to delete {os.path.basename(video_path)}: {e}')
        else:
            self.statusBar().showMessage(f'Kept {os.path.basename(video_path)}.')
