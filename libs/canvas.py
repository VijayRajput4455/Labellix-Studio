...
class Canvas(QWidget):
    ...
    def mouseMoveEvent(self, ev):
        ...
        if Qt.LeftButton & ev.buttons():
            if self.selected_vertex():
                ...
                bounds = self.h_shape.bounding_rect()
                current_width = abs(bounds.width())
                current_height = abs(bounds.height())
                self.parent().window().label_coordinates.setText(
                        'Width: %d, Height: %d / X: %d; Y: %d' % (current_width, current_height, pos.x(), pos.y()))
                ...
                bounds = self.selected_shape.bounding_rect()
                current_width = abs(bounds.width())
                current_height = abs(bounds.height())
                self.parent().window().label_coordinates.setText(
                        'Width: %d, Height: %d / X: %d; Y: %d' % (current_width, current_height, pos.x(), pos.y()))
    ...
    def handle_drawing(self, pos):
        if self.segmentation_mode:
            if self.current:
                ...
                self.current = Shape()
                self.current.max_points = None
                self.current.is_segment = True
                self.current.add_point(pos)
                self.line.points = [pos, pos]
                self.set_hiding()
    ...
    def mouseDoubleClickEvent(self, ev):
        ...
        if self.can_close_shape() and len(self.current) > 2:
    ...
    def paintEvent(self, event):
        ...
        if self.focus_vignette:
            ...
            vignette.setColorAt(0.70, QColor(0, 0, 0, 28))
            vignette.setColorAt(1.00, QColor(0, 0, 0, 120))
    ...
    def set_segmentation_mode(self, status):
        self.segmentation_mode = bool(status)

    def set_focus_vignette(self, status):
        self.focus_vignette = bool(status)
        self.update()
