    ...
    def write(self):
        if os.path.isfile(self.output_file):
            try:
                ...
                LOGGER.exception('Failed to read existing CreateML output file %s', self.output_file)
                raise ValueError('Failed to read CreateML output file %s: %s' % (self.output_file, exc))
        else:
            output_dict = []
        ...
        try:
            atomic_write_text(self.output_file, json.dumps(output_dict), encoding=ENCODE_METHOD)
        except OSError as exc:
            LOGGER.exception('Failed to write CreateML output file %s', self.output_file)
            raise ValueError('Failed to write CreateML output file %s: %s' % (self.output_file, exc))
...
class CreateMLReader:
    def __init__(self, json_path, file_path):
        ...
        try:
            ...
            LOGGER.warning('Failed to parse CreateML annotations from %s: %s', self.json_path, exc)
